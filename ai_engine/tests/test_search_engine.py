...
class TestSearchEngine:
    ...
    def test_search_computers_author_john_debug(self, search_engine):
        """DEBUG: search query='computers' with filter authors='John'"""

        query = "computers"
        filters = {"authors": "Adam"}
        ...
        print(f"[DEBUG] First 20 authors: {filters_data['authors'][:20]}")

        # 2. Build where clause
        where = search_engine._build_where_clause(filters)
        print(f"[DEBUG] Built where clause: {where}")

        # 3. Run search (tăng top_k để tránh bị limit)
        results = search_engine.search(
            query=query,
            filters=filters,
            top_k=50
        ...
        for i, book in enumerate(results):
            ...
            print(f"  Snippet: {book['snippet'][:150]}")
